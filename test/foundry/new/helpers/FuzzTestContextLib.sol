...
struct Expectations {
     ...
     */
    bytes32[] expectedZoneCalldataHashAuthorize;
    bytes32[] expectedZoneCalldataHashValidate;
    /**
...
library FuzzTestContextLib {
    ...
    function empty() internal returns (FuzzTestContext memory) {
        ...
        {
            bytes32[] memory hashesAuthorize;
            bytes32[] memory hashesValidate;
            bytes32[] memory expectedTransferEventHashes;
            bytes32[] memory expectedSeaportEventHashes;

            expectations = Expectations({
                expectedZoneCalldataHashAuthorize: hashesAuthorize,
                expectedZoneCalldataHashValidate: hashesValidate,
